...
describe(ProjectInvitationsApiService.name, () => {
  let service: ProjectInvitationsApiService;
  let httpMock: HttpTestingController;
  let BASE: string;

  beforeEach(() => {
    TestBed.configureTestingModule({ providers: [testingProviders] });
    service = TestBed.inject(ProjectInvitationsApiService);
  ...
  it("list parses conforming payload", async () => {
    const invitation = makeProjectInvitation();
    const promise = lastValueFrom(service.list({ projectId: "project-1" }));

    httpMock.expectOne(`${BASE}/projects/project-1/invitations`).flush({ data: [invitation] });

    expect(await promise).toEqual([invitation]);
  });

  it("list throws on a non-conforming payload", async () => {
    const raw = { ...makeProjectInvitation(), numEmailsSent: "many" };
    const promise = lastValueFrom(service.list({ projectId: "project-1" }));

    httpMock.expectOne(`${BASE}/projects/project-1/invitations`).flush({ data: [raw] });

    await expect(promise).rejects.toThrow();
  });

  it("getByToken parses the public pending invitation", async () => {
    const publicInvitation = makePublicProjectPendingInvitation();
    const promise = lastValueFrom(service.getByToken({ token: "tok-1" }));

    httpMock.expectOne(`${BASE}/projects/invitations/by_token/tok-1`).flush({ data: publicInvitation });
  ...
  it("createBulkInvitations parses the CreateInvitations response", async () => {
    ...
    httpMock.expectOne(`${BASE}/projects/project-1/invitations`).flush(result);
